...
//
// Every claim here comes from foayenix/cv-site (the first twenty entries) or
// from each repo's own README (the six later ones). The wording has been
// edited since; the facts have not. `live` and `source` are only set where the
...
export const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    featured: true,
    oneLiner:
      'A WhatsApp bot that lets African traditional-medicine practitioners document herbal formulations and track patient outcomes by voice and photo. No app to install and nothing to type.',
    signature:
  ...
  },
  {
    ...
    signature:
      'Notes are plain Markdown files plus a single index.json, readable by any tool, with no database anywhere in the design, so the library outlives the app that made it. Storage and capture live in a Swift package with a platform-independent test suite, and the Xcode project is generated from project.yml, because a committed .pbxproj is not reviewable.',
  ...
  },
  {
    ...
    signature:
      'Records form a single graph: a daily entry points at the monograph written that day, which carries sourced claims citing references, which feed the outputs published. Because they connect, one query answers which plants you have written about that have no human-trial evidence and that you have never published on. No server, no account, no sync, no second user.',
